...
public class GaePersistenceManager extends AbstractPersistenceManager {
    ...
    if (!info.aggregatedFields.isEmpty()) {
      for (Field f : info.aggregatedFields) {
        if (ClassInfo.isModel(f.getType())) {
          ...
          if (!lq.asList()
              .isEmpty()) {
            _deleteMultiple(lq.asQuery()
                .fetchKeys(), keys, key, info, f);
          }
        }
      }
    }
  ...
  private <T> void _insertSingle(T obj, final Key parentEntityKey, final Object parentObj, final ClassInfo parentInfo, final Field field) {
    ...
    if (info.hasOwnedFields) {
      ...
      // uses save because we don't know if the objects where already saved or
      // not
      save(relObjects);
    }
  }
  ...
  private <T> int _insertPutEntities(Iterable<Entity> entities, Iterable<T> objects) {
    ...
    for (Object obj : objects) {
      ...
      // Relation rel = new Relation(RelationMode.AGGREGATION, parentObj,
      // field);
  ...
  public void _populateOwnedList(List<Object> relObjects, ClassInfo info, Object obj) {
    for (Field f : info.ownedFields) {
      if (ClassInfo.isOne(f)) {
        ...
        if (!lq.asList()
            .isEmpty()) {
          Field asField = (Field) info.manyFieldMap.get(f)
              .get(FieldMapKeys.FIELD);
  ...
  // private <T> int _updateComplexMultiple(Iterable<T> objs, Key parentKey,
  // ClassInfo parentInfo, Field parentField){
  // HashMap<PersistenceType, List<Entity>> entitiesMap = new
  // HashMap<PersistenceType, List<Entity>>();
  // HashMap<PersistenceType, List<Object>> objectsMap = new
  // HashMap<PersistenceType, List<Object>>();
  // HashMap<PersistenceType, List<Key>> keysMap = new HashMap<PersistenceType,
  // List<Key>>();
  //
  // for(Object obj:objs){
  // _updateBuildMaps(entitiesMap, objectsMap, keysMap, obj, null, null, null);
  ...
  // private void _buildUpdateList(List<Entity> entities2Insert, List<Object>
  // objects2Insert, List<Entity> entities2Update, List<Key> entities2Remove,
  // List<Object> objects2Save, Object obj, Key parentKey, ClassInfo parentInfo,
  // Field parentField){
  private static void _updateBuildMaps(HashMap<PersistenceType, List<Entity>> entitiesMap, HashMap<PersistenceType, List<Object>> objectsMap, HashMap<PersistenceType, List<Key>> keysMap, Object obj,
    ...
    for (Field f : info.aggregatedFields) {
      if (ClassInfo.isOne(f)) {
        ...
        /*
         * if(!lq.asList().isEmpty()){ for(Object elt : lq.asList()){
         * _buildUpdateList(entities, entities2Remove, objects2Save, elt,
         * entity.getKey(), info, f); } }
         */
    ...
    try {
      ...
      for (final T model : models) {
        for (Field field : fieldMap.keySet()) {
          ...
          // our object is not linked to another object...so it doesn't have any
          // key
          if (objVal == null) {
            continue;
          }
  ...
  protected <T> void fillAggregated(ClassInfo info, T ancestor, Key ancestorKey) {
    // now gets aggregated one2one (one2many are retrieved by ListQuery except
    // with @Join)
    for (Field f : info.aggregatedFields) {
      ...
      // todo manage joined one2many listquery
        ...
        // sets the sync flag to false to tell that it should be fetched when
        // the listquery is accessed!
        lq.setSync(false);
      }
    }
  }
  ...
  protected <T> T mapAggregated(T model) {
    ...
    /*
     * if(!hasOneMany){ // creates a kindless query to retrieve all subentities
     * at once. com.google.appengine.api.datastore.Query q = new
     * com.google.appengine.api.datastore.Query(); Key parentKey =
     * GaeMappingUtils.getKey(model);
     * 
     * q.setAncestor(parentKey); // this removes the parent from query
     * q.addFilter(Entity.KEY_RESERVED_PROPERTY,
     * com.google.appengine.api.datastore.Query.FilterOperator.GREATER_THAN,
     * parentKey);
     * 
     * PreparedQuery pq = ds.prepare(q); List<Entity> childEntities =
     * pq.asList(FetchOptions.Builder.withDefaults());
     * 
     * for(Field f: modelMap.keySet()){ Class<?> fClazz = f.getType(); ClassInfo
     * fInfo = modelMap.get(f); String kind =
     * GaeMappingUtils.getKindWithAncestorField(fInfo, info, f); Entity found =
     * null; for(Entity e: childEntities){ if(kind.equals(e.getKind())){ found =
     * e; childEntities.remove(e); break; } }
     * 
     * if(found != null){ Object fObj = GaeMappingUtils.mapEntity(found,
     * fClazz); Util.setField(model, f, fObj); } } } else { for(Field f:
     * modelMap.keySet()){ Class<?> fClazz = f.getType(); ClassInfo fInfo =
     * modelMap.get(f); String kind =
     * GaeMappingUtils.getKindWithAncestorField(fInfo, info, f);
     * com.google.appengine.api.datastore.Query q = new
     * com.google.appengine.api.datastore.Query(kind); Key parentKey =
     * GaeMappingUtils.getKey(model); q.setAncestor(parentKey); PreparedQuery pq
     * = ds.prepare(q); Entity childEntity = pq.asSingleEntity(); Object fObj =
     * GaeMappingUtils.mapEntity(childEntity, fClazz); Util.setField(model, f,
     * fObj); } }
     */
    return model;
  }
  ...
  protected <T> T map(Query<T> query, Entity entity) {
    ...
    // join management
    if (!query.getJoins()
        .isEmpty() || !ClassInfo.getClassInfo(clazz).joinFields.isEmpty())
  ...
  protected <T> List<T> map(Query<T> query, List<Entity> entities) {
    ...
    // join management
    if (!query.getJoins()
        .isEmpty() || !info.joinFields.isEmpty())
  ...
  private <T> List<T> doFetchList(Query<T> query, int limit, int offset) {
    ...
    if (!pag.isPaginating()) {
      ...
      if (pag.isActive()) {
        ...
        // using pageOption.pageSize
          ...
          fetchOptions.limit(gaeCtx.realPageSize);
          // passivates the pageOption in stateless mode not to keep anything
          // between 2 requests
    ...
    // if previousPage has detected there is no more data, simply returns an
    // empty list
    ...
    if (state.isStateless()) {
      ...
      case KEYS_ONLY: {
        // uses iterable as it is the only async request for prepared query for
        // the time being
        List<Entity> entities = prepareKeysOnly(query).asList(fetchOptions);
        // if paginating and 0 results then no more data else resets
        // noMoreDataAfter
        if (pag.isPaginating()) {
          if (entities.size() == 0) {
      ...
      if (!gaeCtx.isActive()) {
        ...
        case KEYS_ONLY: {
          ...
          gaeCtx.activate();
          // sets the current cursor (in stateful mode, cursor is always kept
          // for further use)
          if (pag.isPaginating()) {
            ...
            Cursor cursor = entities.getCursor();
            if (cursor != null) {
              gaeCtx.addAndMoveCursor(entities.getCursor()
                  .toWebSafeString());
            }
    ...
    if (state.isStateless()) {
      ...
      if (!gaeCtx.isActive()) {
        ...
        default: {
          ...
          if (pag.isPaginating()) {
            // in case of pagination, we need to allow asynchronous calls such
            // as:
            // QueryAsync<MyClass> query =
            // pm.createQuery(MyClass).paginate(5).stateful().order("name");
            ...
            // so we can't use the asQueryResultIterable as the cursor is not
            // moved to the end of the current page
            // but moved at each call of iterable.iterator().next()
            // thus we use the List in this case to be able to move directly to
            // the next page with cursors
            QueryResultList<Entity> entities = pq.asQueryResultList(fetchOptions);
            ...
            // if not paginating, we simply use the queryresultiterable and
        ...
        default: {
          ...
          if (pag.isPaginating()) {
            ...
            if (!gaeCtx.useCursor) {
              ...
              // sets the current cursor (in stateful mode, cursor is always
              // kept for further use)
              // if(gaeCtx.useCursor){
              gaeCtx.addCursor(entities.getCursor()
                  .toWebSafeString());
              // }
            }
            return new GaeSienaIterable<T>(this, entities, query);
  ...
  public <T> List<T> fetch(Query<T> query, int limit, Object offset) {
    ...
    // (QueryOptionOffset)query.option(QueryOptionOffset.ID);
  ...
  public <T> int delete(Query<T> query) {
    ...
    if (!info.hasAggregatedFields) {
      ...
      // if aggregated fields, needs to retrieve all objects to scan & delete
      // aggregated children
  ...
  static {
    supportedOperators = GaeQueryUtils.operators.keySet()
